...
glop <- read.csv(file.path(data.dir, "glopnet.csv"))
...
ll.max = which(glop$ll == max(glop$ll, na.rm = T))
glop[ll.max, c("biome", "ll")]

# (2) What is the species with the longest leaf lifespan?

glop[ll.max, c("species", "ll")]
...
glop[glop$biome == "ALPINE" | glop$biome == "TUNDRA", c("log.ll", "biome")]
...
glop$biome.type = ""
glop$biome.type[glop$biome %in% forests] = "forest"
glop$biome.type[glop$biome %in% not.forests] = "not.forest"
