...
# and change the second to last column name to "ca.ci". Make a real 'species' column. Also
# add a leaf lifespan column back-transformed to months.
...
# under << DATASETS >> .)
t1.lidar <- as.matrix (read.csv (file.path (dat.dir, "lidar", "lidar_t1.csv")))
...
#--View the head of our 'trait.cols' columns in 'glop' ('trait.cols' was made in matrices
# section above).
trait.cols
...
paste (dwarves [["names"]], dwarves [["sex"]])
tmp = c("names", "sex")
paste(dwarves[glop[[c("names", "sex")]]])
...
lmod = lm(glop[[trait.a]] ~ glop[[trait.b]])
...
survey.1$t2.lidar <- as.matrix (read.csv (file.path (dat.dir, "lidar", "lidar_t2.csv")))
...
survey.1$t3.lidar <- as.matrix (read.csv (file.path (dat.dir, "lidar", "lidar_t3.csv")))
survey.1$t4.lidar <- as.matrix (read.csv (file.path (dat.dir, "lidar", "lidar_t4.csv")))
...
# NAs.
rowSums(survey.1$t1.lidar)
for (i in 1:4) {
  survey_fld = paste0("t", i, ".lidar")
  sum_fld = paste0("t", i, ".sum")
  means_fld = paste0("t", i, ".mean")
  survey.1$stats[[sum_fld]] = rowSums(survey.1[[survey_fld]], na.rm=T)
  survey.1$stats[[means_fld]] = rowMeans(survey.1[[survey_fld]], na.rm=T)
}
...
# NA and NaN
#-----------------------------------------------------------------------------------------
# Missing or undefined values are designated in R by NA and NaN ("not a number"). Your
# indexing troubles will often be related to incorrect handling of NAs.

#--A temporary data frame for illustration:
tmp <- data.frame (one=c('a',NA,'c','d'), two=c(1,NA,NaN,4))
tmp
# NOTE: Get in the practice of making little test data frames that contain the structures
# that will make or break your code, and test your code on them.

# << Identifying missing values >> -------------------------------------------------------

#--Both NA and NaN can be identified in vectors with the logical test is.na().
is.na (tmp$one)
is.na (tmp$two)
# Notice how the NaN also returns TRUE with is.na(), because it is a form of NA.

#--To look for NaN only, use is.nan(). NaN can be produced by mathematical processes like
# 0/0, so you might want to identify them separately from missing values.
is.nan (tmp$two)

# NOTE: NAs can only be identified by is.na()!  x==NA does not work because NA is not a
# value that can be logically tested, it is a special kind of element: a lack of value.

# << NAs and indexing >> -----------------------------------------------------------------

#--Ever gotten back a data frame full of NAs from your indexing? This is why:
tmp [tmp$two > 4, ]
# This happens when you do a logical test on an NA. Asking "is NA greater than 4?" makes
# no sense, so R says "I pass" and returns and NA. If that NA is used to index a row, you
# get at least one row of NAs. Try it.
tmp [NA, ]
# or
tmp [c(F,NA,NA,F), ]
# Or if some of your tests pass, but the others act on NAs:
tmp [tmp$two < 3, ]

#--Avoiding NAs with which():
# which() returns the integer positions of TRUE values in a logical string. It therefore
# ignores NAs returned by the test. Piecing together an index from the inside out:
tmp$one == "c"
which (tmp$one == "c")
tmp [which (tmp$one == "c"), ]
# It also ignores NaNs.
tmp [which (tmp$two > 0), ]

#--Avoiding NAs with !is.na():
# Sometimes you want your index to stay logical, and not be converted to an integer with
# which(). For example, for creating a logical indexing object that you want to make
# interact with other logical indexing objects using & or |.
# - Use !is.na() to return TRUE for every non-NA value.
is.na (tmp$two)
! is.na (tmp$two)
tmp [! is.na (tmp$two), ]

#--Illustration of dealing with NAs in logical indexing objects:
# - Make a logical vector for indexing long-lived leaves in 'glop', avoiding the NAs.
long.lived <- glop$ll > 24 & !is.na (glop$ll)
# - Make a logical vector for indexing data in the TUNDRA biome. (No biome entries are NA)
tundra <- glop$biome == "TUNDRA"
# - Now make those interact to identify the unique tundra genera in 'glop' with long-lived
# leaves.
unique (glop [long.lived & tundra, "genus"])
# You could leave the NAs in your logical vectors and wrap long.lived&tundra inside of
# which() to deal with them in the index, but the method showed here uses index code
# that's a little more compact and easily interpreted.

#--na.omit()
# This will return an entire data frame without the rows containing NAs.
na.omit (tmp)
# Any indexing method that returns a data frame can be used with na.omit().
na.omit (glop [c('log.ll','log.lma')])

#--complete.cases()
# A logical test on a single vector or a collection like columns in a data frame. Returns
# TRUE where all elements in a row are NOT NA. Can therefore be used to index.
complete.cases (tmp$two)
complete.cases (tmp)
tmp [complete.cases(tmp), ]
# - View all of the glop data for the TROP_RF biome where both 'log.ll' and 'log.lma'
# columns have data.
glop [glop$biome == "TROP_RF" & complete.cases (glop [c('log.ll','log.lma')]), ]

#--%in% and NAs.
# x %in% y is much like x == y[1] | x == y[2] | x == y[...] except for one important
# difference. When y[i] is NA, x == y[i] returns NA, which messes up your index. However,
# x %in% y[i] returns FALSE if there are no NAs in x. If there are NAs in x, %in% will
# recognize matches to NAs in y if there are any, and return TRUE.
# - Try both ways for indexing 'a' entries in tmp$one.
tmp [tmp$one=='a', ]
tmp [tmp$one %in% 'a', ]

# << NAs in functions >> -----------------------------------------------------------------

#--Built-in functions with argument na.rm=T
# Many built-in functions have an argument for specifying what to do with NAs.
max (tmp$two)
max (tmp$two, na.rm=T)

# Other functions use an argument na.action, in which you can specify the different NA
# options listed in the help for ?na.omit .
...
# - Try the variant grepl() for logical.
grepl ("Mex", forest.plots$plot)
...
#-----------------------------------------------------------------------------------------
# Essential for anybody trying go big and creative with data analysis!!

# The arguments: merge (x, y, by, all)
# 'x' and 'y' are the datasets to be merged. 'by' specifies the column of data in common.
# 'all' specifies whether to include data from both datasets for all possibilities of
# 'by'.

# << Example 1 >> ------------------------------------------------------------------------
# Say you collected species and trait data at some sites, then later you got environmental
# data for those sites. You'd like to compare metrics from your plot measurements to the
# environment.
# - The plot data (don't worry about this code, we're just making something up to look at)
plots <- data.frame (plot = c(rep('plot1',3),rep('plot2',8),rep('plot3',5)),
                     species = glop$species[1:16], trait = rnorm(16,5,3))
plots
# - The environment data.
env <- data.frame (plot = paste0 ("plot",1:5), precip = rnorm(5,1000,200))
env
# The plot data and the environment data have a variable in common, 'plot'.
# - To get the precip data into your plot data, redefine the plot data as itself merged
# with the environment data.
plots <- merge (plots, env, by="plot")
plots
# Notice that 'env' has plot4 and plot5, which don't exist in 'plots'. They didn't get
# merged because the default in merge() is all=FALSE. Delete that precip column and try
# again with argument all=TRUE.
plots <- plots [!names(plots)=="precip"]
plots <- merge (plots, env, by="plot", all=T)
plots
# Now we have plots 4 and 5 with their precip data, and NAs for the rest. You can specify
# to keep all of 'x' (most common) with all.x=T, or all of 'y' with all.y=T.
# - You'd more likely merge in meta data after getting some single metric calculated for
# each plot. Here's an example.
plots <- plots [!is.na (plots$trait), ]
plots
plot.trait.means <- aggregate (
  plots$trait, by=list(plots$plot),function (x) mean (x, na.rm=T))
names (plot.trait.means) = c("plot","trait.mean")
plot.trait.means
# Now merge in the meta data.
plot.trait.means <- merge (plot.trait.means, env, by="plot", all.y=F)
plot.trait.means
# Now you can test relationships between plot trait means and environmental data!

# << Example 2 >> ------------------------------------------------------------------------
# Our glopnet data lacks plant family names. Fortunately, there is an online taxonomy
# service that will give us family names for a list of plant species names. The dataset
# 'glopnet.tnrs.csv' included with these tutorials has the output from that service. (The
# Taxonomic Name Resolution Service, or TNRS, does much more than return families, check
# it out if you work with plants).

#--Let's get those family names merged in. Call in the TNRS dataset.
tnrs <- read.csv (paste0 (dat.dir, "glopnet.tnrs.csv"))
# - Change the 'genus sp' separator in the tnrs to "_" with gsub().
tnrs$Name_submitted <- gsub (" ", "_", glop.tnrs$Name_submitted)

# 1: We don't need all the TNRS columns, we just need the one with submitted species names
# (to match to glop names), and the column with the family names. So just index those
# columns in the 'y' argument.
# 2: All glop species names should match the tnrs submitted names, but just in case, use
# 'all.x=T' to make sure we don't lose any data.
# 3: The columns containing the species names to merge by have different names in each
# dataset. So specify the different names with by.x and by.y.
glop <- merge (glop, tnrs [c("Name_submitted","Accepted_family")], 
               by.x="species", by.y="Name_submitted")
# Now we've got family data correctly aligned with each species! (at least each that
# matched in the TNRS databases.)
...
# ?subset. The arguments are:  subset (x, subset, select, ...) 'x' is the object (data) to
# be subsetted. Subset is an argument that produces a logical vector.  Select is which
# columns to keep.
...
#--Make a subsetted dataframe 'glop.sub' with all of the TUNDRA data. Leave the 'select'
# argument blank so we get all the columns back.
glop.sub <- subset (x = glop, subset = biome=="TUNDRA")

#--Make the subset the taxonomy data only, for lifespans greater than 2 years (ll>24). Do
# not "quote" the column names for 'select', or for subset.
glop.sub <- subset (x = glop, subset = ll > 24, select = c(genus, species))

# If you check that returned data frame, you'll find there are no NAs, even though some of
# the test ll>24 will have returned NAs. subset() treats NAs as false.

# NOTE: I never use subset(). Like many functions, it is a convenient wrapper for
# something you could do manually. In this case, regular old indexing does the same thing
# and doesn't require special knowledge of the syntax of a particular function, and
# nothing is left occurring inside a 'black box'.
