...
dat.dir <- "directory path to your data folder ending with forward slash/"

#--Make a figures folder somewhere and define the path in the object 'fig.dir'.
fig.dir <- "directory path to your figures folder ending with forward slash/"
...
# for-loop structure.
for (i in seq_along (vec)) { vec[i] = vec[i] * 2 }
vec
...
#--ANSWER IS AT BOTTOM OF TUTORIAL.
...
colnames.vec <- c("dataset", "biome", "genus", "species")
for (cn in colnames.vec) { print (length (unique (glop [[cn]] ))) }
...
# POP QUIZ 2: Which biomes produced NaN results? Why did they return NaN?
...
# 'sd' equal to i+2.
for (i in 1:10) { print (rnorm (n=10, mean=i, sd=i+2)) }
...
biome.results <- data.frame (biome=biome.list, mean.log.ll=NA)
...
for (bn in biome.list) {
  mean.i <- mean (glop [glop$biome == bn, "log.ll"], na.rm=T)
  biome.results [biome.results$biome == bn, "mean.log.ll"] = mean.i
}
...
for (i in letters.vec) {
  print (i)
  for (j in numbers.vec) {
    print (j)
  }
}
...
for (b in biome.list) {
  for (phen in c("E","D")) {
    #--Count the non-NA values of 'log.lma' in biome 'b' and decid.e.green 'phen'.
    count.i <- length (which (!is.na (
      glop [glop$biome == b & glop$decid.e.green == phen, "log.lma"] )))
    #--Deposit the count into 'lma.counts' in the right place.
...
x = 2
if (x+x == 2 | x+x ==4) { print ("it's true!") }
...
for (i in 1:3) {
  if (i == 1) {
    print ("i equals 1")
  } else if (i == 2) { print ("i equals 2")
  } else { print ("i does not equal 1 or 2") }
  print (paste ("test", i, "complete"))
}
...
test.families <- c("Caprifoliaceae","Lauraceae","Podocarpaceae")

for (biome in biome.list) {
  test <- test.families %in% glop [glop$biome==biome, "family"]
  if (all (test)) {
    print (paste ("All of the test families occur in biome", biome))
    next
  } else if (any (test)) {
    print (paste (
      "Not all, but at least one of the test families occurs in biome", biome))
  } else { print (paste ("None of the test families occur in biome", biome)) }
}
...
for (biome in biome.list) {
  ...
  lm <- lm (log.narea ~ log.lma, data=dat)
  #--Save the p, rsq, intercept, and slop values to objects, truncated to 2 significant
  # digits.
  p <- signif (summary(lm)$coefficients [8], digits=2)
  rsq <- signif (summary(lm)$adj.r.squared, digits=2)
  intercept <- signif (lm$coefficients [1], digits=2)
  slope <- signif (lm$coefficients [2], digits=2)

  #--Use ggplot2 to make the scatter plot.
  g <- ggplot (dat, aes (x=log.lma, y=log.narea)) + geom_point (color="blue")
  # Add the regression line.
  g <- g + geom_abline (intercept=intercept, slope=slope, color="red")
  # paste0() together a title indicating the biome, p, and rsq value.
  title <- paste0 ("Biome: ", biome, ". p = ", p, ". r-squared = ", rsq, ".")
  ...
  # name.
  png (file = paste0 (fig.dir, biome, ".png"), width=600, height=600)
...
# correspond to traits 'log.ll', 'log.lma', 'log.amass', and 'log.aarea'. Get the column
# numbers using which() and %in%.
t.col.nums <- which (names (glop) %in% c('log.ll','log.lma','log.amass','log.aarea'))

# POP QUIZ 2: Which biomes produced NaN results? Why did they return NaN?
#--The third and the ninth results were NA. See which biomes those are:
biome.list [3]; biome.list [9]
#--Check whether there is any non-NA data for 'log.ll' in those biomes.
length (which (!is.na (glop [glop$biome=="DESERT", "log.ll"])))
length (which (!is.na (glop [glop$biome=="TEMP_RF", "log.ll"])))
# They both have zero non-NA data, so the mean is NA. You could create a loop to count the
# non-NA data in each group. That's a really useful first pass exploration of your data!
