...
text.1 <- "this is a character string"
text.1
...
vector.three <- c(vector.one, vector.two)
vector.three
...
# vector.
vector.four <- c(1, 2, vector.two*2, 7:10, c(1,2,3))
vector.four
...
prod <- vector.one[3] * vector.two[2]
prod
...
# - Pop quiz answers at bottom of tutorial.

# >> You get "20" repeated 20 times because the object "a" is set to 20
# rep(a, 20)
...
glop <- read.csv (file="glopnet.csv")
...
glop <- read.csv (file = paste0 (dat.dir, "glopnet.csv"))
...
# POP QUIZ 1: What happens if you DON'T put the quotes around "a"? Why did that happen?
# - Without quotes, R will look for an object named 'a' instead of the letter "a". If that
# object doesn't exist, R will return an error because it can't find 'a'.

# POP QUIZ 2: What unit is considered a single 'element' of df1? (Go back to the definition
# of data frames at the beginning of this Part.)
# - Data frames are 'lists' of 'vectors'. In other words, columns are vectors of equal
# length, and data frames are collections of columns. So a single 'element' of df1 is a
# column.
