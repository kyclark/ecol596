...
dat.dir <- "/Users/ttaylor/Documents/Stats and R course/Tutorial datasets/"
...
print_sqrt <- function (x) {
  print (paste0 ("The square root of ", x, " is ", sqrt(x), "."))
}
...
graph_traits <- function (biome, trait.1, trait.2) {
  ...
  title <- paste0 (trait.1, " vs. ", trait.2, " for biome ", biome, ".")
...
graph_traits <- function (biomes="all", trait.1, trait.2, complete.traits=NULL) {
  ...
  #--Make the graph title as an object.
  title <- paste0 (trait.1, " vs. ", trait.2, ".")
  #--Make the plot in the usual base-R way. You don't need the print() function because
  # plot() already commands export to the plotting window.
  plot (x=dat[[trait.1]], y=dat[[trait.2]], main=title, xlab=trait.1, ylab=trait.2)
}
...
#--Let's make the function to get a jacknife estimate of the slope of the relationship
...
jack_slope <- function (trait.x, trait.y, group) {
  ...
  for (element.i in unique (glop [[group]])) {
...
jack.biome.table <- jack_slope ('log.lma', 'log.ll', 'biome') [[2]]
jack.biome.est <- jack_slope ('log.lma', 'log.ll', 'biome') [[1]]
...
jack_slope <- function (trait.x, trait.y, group) {
  ...
  if (any (test)) {
    warning ("At least one removal group has zero complete cases of trait data.")
  }
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  for (element.i in unique.elements) {
    dat <- glop [!glop[[group]]==element.i & !is.na (glop[[group]]), ]
...
jack_slope <- function (trait.x, trait.y, group) {
  ...
  # vector with TRUE representing biomes that had no complete cases.)
  if (any (test)) {
    stop ("At least one removal group has zero complete cases of trait data.")
  }
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  for (element.i in unique.elements) {
    dat <- glop [!glop[[group]]==element.i & !is.na (glop[[group]]), ]
...
fun.dir <- "/Users/ttaylor/Documents/Stats and R course/Tutorials/Tutorial 4/"
...
source (paste0 (fun.dir, "wrap_text.R"))
...
graph_traits <- function (biomes="all", trait.1, trait.2, complete.traits=NULL) {
  ...
  if ("all" %in% biomes) {
    title.biomes <- "All"
  } else { title.biomes <- paste (biomes, collapse="; ") }
  #--Make the graph title as an object.
  title <- paste0 (trait.1, " vs. ", trait.2, " for biome(s): ", title.biomes, ".")
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  #--Make the plot in the usual base-R way. You don't need the print() function because
...
wrap_text (text=my.text, wrap.length=30)
...
# Examine and this modification of the graph_traits() function, which we created
# in Part 5. It has been modified to include the '...' argument in two places: (1) at the
# end of the graph_traits() arguments, and (2) in the call to wrap_text(), commented
# below.
...
graph_traits (
  biomes=c('ALPINE','TEMP_FOR','DESERT','GRASS/M','TROP_FOR'), "log.lma", "log.ll",
  wrap.length=30)
...
graph_traits <- function (
  biomes="all", trait.1, trait.2, complete.traits=NULL, wrap.length=40) {
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  if (! "all" %in% biomes) { dat <- glop [glop$biome %in% biomes, ] } else { dat <- glop }
  if (!is.null (complete.traits)) { dat <- dat [complete.cases (dat[complete.traits]), ] }
  if ("all" %in% biomes) {
  ...
  title <- wrap_text (text=title, wrap.length=wrap.length)
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  p <- ggplot (dat, aes(trait.1, trait.2)) + geom_point() + ggtitle (title)
  print (p)
}
