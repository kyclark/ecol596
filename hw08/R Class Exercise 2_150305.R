...
for (s in 1:2){
  lidar[[s]] = list()
  for (t in 1:4){
    file = file.path(dat.dir, paste("Lidar_Survey", s), sprintf("lidar_s%s_t%s.csv", s, t))
    if (file.exists(file)) {
      print(paste("Processing file = ", file))
      lidar[[s]][[t]] = as.matrix(read.csv(file))
    }
  }
}
...
lidar<-list()
for (s in 1:2){
  sname = paste0('survey.', s)
  lidar[[sname]] = list()
  for (t in 1:4) {
    file = file.path(dat.dir, paste("Lidar_Survey", s), sprintf("lidar_s%s_t%s.csv", s, t))
    print(paste('sname', sname))
    if (file.exists(file)) {
      print(paste("Processing file = ", file))
      tname = sprintf("lidar.s%s.t%s", s, t)
      lidar[[sname]][[tname]] = as.matrix(read.csv(file))
    }
  }
}
...
for (sname in names(lidar)) {
  for (tname in names(lidar[[sname]])) {
    lidar[[sname]][[tname]][ lidar[[sname]][[tname]] == -9999 ] = NA
  }
}
...
library(fields)
for (s in names(lidar)) {
  for (t in names(lidar[[s]])) {
    plot.title = paste(s, t, sep="_")
    print(plot.title)
    png(filename=file.path(res.dir, paste0(plot.title, '.png')), width=1000, height=600)
    image.plot(t(lidar[[s]][[t]]), main=gsub("_", " ", plot.title))
    dev.off()
  }
}
...
for (s in c(1, 2)) {
  transect.summary = data.frame(height=1:60)
  
  for (t in c(1:4)) { 
    nrows = nrow(lidar[[s]][[t]])
    sums  = rowSums(lidar[[s]][[t]], na.rm=T) 
    means = rowMeans(lidar[[s]][[t]], na.rm=T) 
    maxs  = apply(lidar[[s]][[t]], 1, function(x) { max(x, na.rm=T) })

    df = data.frame(maxs, sums, means)
    colnames(df) = c(
      paste0('t', t, '.max.lad'), 
      paste0('t', t, '.sum.lad'), 
      paste0('t', t, '.mean.lad'))
    
    transect.summary = cbind(transect.summary, df)
  }
  
  lidar[[s]][['transect.summary']] = transect.summary
}