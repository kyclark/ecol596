...
head( glop [ , - which (names (glop) == "biome" | names(glop) == "log.rdmass" | 
                           names(glop) == "angio")])
...
# column.
which(glop$genus %in% "Rosa")

which(glop$genus == "Rosa")

glop[which(glop$genus %in% c("Smilax", "Rosa", "Adenostoma")), "genus"]
...
names(glop) [names (glop) %in% c("code", "log.rdmass", "biome")]
...
wanted = c("bashful", "grumpy", "sleepy")
ages[names %in% wanted]
